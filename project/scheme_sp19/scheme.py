...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        return None
    tmp_exp, next_exp = expressions.first, expressions.second
    while next_exp is not nil:
        scheme_eval(tmp_exp, env)
        tmp_exp, next_exp = next_exp.first, next_exp.second
    return scheme_eval(tmp_exp, env, True)
    # END PROBLEM 8
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        ...
        while expressions is not nil:
            tail = expressions.second is nil
            val = scheme_eval(expressions.first, env, tail)
            if scheme_truep(val):
