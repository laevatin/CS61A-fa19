...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        nearest = []
        distance = 0
        while search and search.name != 'Hive':
            if search.bees and (self.min_range <= distance and distance <= self.max_range):
                nearest = search.bees
                break
            search = search.entrance
            distance += 1
        return random_or_none(nearest)
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if amount >= self.armor:
            reflect_dmg = self.armor + self.damage
        else:
            reflect_dmg = amount
        for bee in self.place.bees[:]:
            bee.reduce_armor(reflect_dmg)
...
# BEGIN Problem 8
class WallAnt(Ant):
    """Wallant has a large armor value and does nothing each turn."""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower will not lose armor when placed in water"""
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.fake:
            ...
            while search:
                if search.ant:
                    if search.ant not in QueenAnt.buffed:
                        QueenAnt.buffed.append(search.ant)
                        search.ant.damage *= 2
                    if search.ant.is_container:
                        if search.ant.contained_ant and search.ant.contained_ant not in QueenAnt.buffed:
                            QueenAnt.buffed.append(search.ant.contained_ant)
                            search.ant.contained_ant.damage *= 2
                search = search.exit
