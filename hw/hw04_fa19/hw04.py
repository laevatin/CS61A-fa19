...
def balanced(m):
    ...
    if is_weight(m):
        ...
        t_left = length(left_m) * total_weight(end(left_m))
        t_right = length(right_m) * total_weight(end(right_m))
        if t_left == t_right:
            return all([balanced(end(left_m)), balanced(end(right_m))])
        else:
            return False
def totals_tree(m):
...
def make_fib():
    ...
    True
    """
    fib_now, fib_prev1, fib_prev2, count = 1, 1, 0, 0
    def fib():
        nonlocal fib_now, fib_prev1, fib_prev2, count
        if count == 0:
            count += 1
            return 0
        elif count == 1:
            ...
            fib_now = fib_prev1 + fib_prev2
            fib_prev1, fib_prev2 = fib_now, fib_prev1
            return fib_now
    return fib
    # clear version #
    # current, next = 0, 1
...
def make_withdraw(balance, password):
    ...
    wrong_pw = []
    locked = False
    def withdraw(amount, pw_):
        nonlocal money, pw, locked
        if not locked:
            if pw == pw_ and money >= amount:
                money -= amount
                return money
            elif pw == pw_:
...
def make_joint(withdraw, old_password, new_password):
    ...
    def joint(amount, pw_):
        if pw_ == old_password or pw_ == new_password:
            return withdraw(amount, old_password)
        else:
            return withdraw(amount, pw_)
    return joint
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert lower_bound(y) > 0 or upper_bound(y) < 0, "You should not divide an interval contains zero"
...
def quadratic(x, a, b, c):
    ...
    extremum = - b / (2 * a)
    if lower_bound(x) < extremum and extremum < upper_bound(x):
        minimum = min(f(lower_bound(x)), f(upper_bound(x)), f(extremum))
        maximum = max(f(lower_bound(x)), f(upper_bound(x)), f(extremum))
        return interval(minimum, maximum)
    else:
        minimum = min(f(lower_bound(x)), f(upper_bound(x)))
        maximum = max(f(lower_bound(x)), f(upper_bound(x)))
        return interval(minimum, maximum)
